...
```{r yti3a-check}
grade_result(pass_if(~identical(.result, 84))) 
```
...
```{r yti4ai-check}
grade_this(pass_if_equal(x = round(.result,3),y = .006 )) 
```
...
```{r yti4aii-check}
grade_this(pass_if_equal(x = round(.result,1),y = round(8.49986e-06,1))) 
```
