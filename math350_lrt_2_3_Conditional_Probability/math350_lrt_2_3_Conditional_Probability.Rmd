...
Suppose that, under close examination, we know that it will be an early bulb. What is the probability that, given we have an early bulb, it is a red tulip? (Use `R` as a calculator here. Keep three decimal places).
...
### Example
Suppose that $P(A) =.3, P(B)=.7$, and $P(A \cap B)=.2$. What is $P(A|B)$? (Use `R` as a calculator here) *NEED to specify enter fraction*s
...
```{r ex3-check}
grade_this(if (setequal(.result, solution)) {
    pass("Nice!")
}
else{
  fail()
...
### You try it
1. Suppose that $P(A)=.7, P(B)=.5$, and $P(A \cap B)=.2$. Find $P(A|B)$. (Use `R` as a calculator here) *NEED to specify enter fraction*
...
```{r yti1b-check}
grade_this(pass_if_equal(x = round(.result,1), y = .1))
```
