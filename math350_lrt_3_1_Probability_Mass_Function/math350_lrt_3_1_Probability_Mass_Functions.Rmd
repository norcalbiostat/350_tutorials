---
title: 'Section 3.1: Probability Mass Functions'
output: 
    learnr::tutorial:
      progressive: true
      allow_skip: true
runtime: shiny_prerendered
description: > 
---

```{r setup, include=FALSE}
library(learnr)
library(gradethis)
library(learnrhash)
library(shiny)
gradethis::gradethis_setup()
knitr::opts_chunk$set(echo = FALSE)
tutorial_options(
  exercise.blanks = TRUE, 
  exercise.reveal_solution = FALSE
)
```

### Example 

```{r quiz1, echo = FALSE}
quiz(caption ="Consider an experiment that is the single roll of a die, where the number of spots on the face up side of the die when rolled is observed.",
question(
  "Outcome space:",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
),
question(
  "Space of the random variable $X$:",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
)
)

```


```{r quiz2, exercise.reveal_solution=TRUE}
quiz(caption = "Dr. D has 2 dogs and 2 cats in her household, so S = {cat, dog}. Let $Y$ be a random variable that denotes the type of animal. $Y$ then maps each element in $S$ to one and only one real number:",
question(
  "When the sample space only has two outcomes,",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
))

```

### Example 

```{r quiz3, echo = FALSE}
quiz(caption = "Consider a crooked dice where the cube is shortened in the one-six direction. This has the effect that 1's and 6's have a probability of 1/4 of being rolled, where the other faces each have a probability of 1/8.",
question(
  "Define the random variable",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
),
question(
  "What is the probability distribution.",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
),
question(
  "Is this a valid pmf?  Explain.",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
)
)

```

### You try it

Suppose your roll 2 dice. Let $X$ be the sum of the two die. Write out the pmf. Don't forget you can refer to Example 2.8 in the textbook to visualize the sample space. 

```{r quiz4, echo = FALSE}
question(
  "Suppose your roll 2 dice. Let $X$ be the sum of the two die. Write out the pmf. Don't forget you can refer to Example 2.8 in the textbook to visualize the sample space. ",
  answer("NO1", correct = TRUE),
  answer("NO2"),
  answer("NO3"),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
)

```

### Example
Suppose your roll 2 dice. Let $X$ be the sum of the two die. Use simulation to estimate the probability distribution.

```{r ex4a, exercise = TRUE}
die <- 1:6 
d1  <- sample(die, 1000, replace = TRUE)
d2  <- sample(die, 1000, replace = TRUE)
sum.2d6   <- d1 + d2
```

```{r ex4a-solution}
die <- 1:6 
d1  <- sample(die, 1000, replace = TRUE)
d2  <- sample(die, 1000, replace = TRUE)
sum.2d6   <- d1 + d2
```

```{r ex4a-check}
grade_code()
```

The pmf of $X$ is: 

```{r ex4b, exercise = TRUE}
die <- 1:6 
d1  <- sample(die, 1000, replace = TRUE)
d2  <- sample(die, 1000, replace = TRUE)
sum.2d6   <- d1 + d2
proportions(table(sum.2d6))
```

```{r ex4b-solution}
die <- 1:6 
d1  <- sample(die, 1000, replace = TRUE)
d2  <- sample(die, 1000, replace = TRUE)
sum.2d6   <- d1 + d2
proportions(table(sum.2d6))
```

```{r ex4b-check}
grade_code()
```

### Plotting the pmf
We can use the function `plot` to plot the estimate of the pmf using the following code. Delete the `#` to see.

```{r plot1, exercise = TRUE}
die <- 1:6 
d1  <- sample(die, 1000, replace = TRUE)
d2  <- sample(die, 1000, replace = TRUE)
sum.2d6   <- d1 + d2
#plot(proportions(table(sum.2d6)), main="Sum of two dice", ylab="Probability")
```

```{r plot1-solution}
die <- 1:6 
d1  <- sample(die, 1000, replace = TRUE)
d2  <- sample(die, 1000, replace = TRUE)
sum.2d6   <- d1 + d2
plot(proportions(table(sum.2d6)),main="Sum of two dice", ylab="Probability")
```

```{r plot1-check}
grade_code()
```

### You try it

1. Three coins are tossed and the number of heads $X$ is counted.

```{r theoretical, echo = FALSE}
question("Write out the theoretical pmf for $X$",
  answer("NO1"),
  answer("NO2"),
  answer("NO3", correct = TRUE),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
)
```

Confirm via simulation.
*DR D*
```{r yti2a, exercise = TRUE}

```

```{r yti2a-check}
coin <- c("h","t")

coin_toss <- replicate(10000,{
  results <- sample(coin,5,replace=TRUE)
  sum(results=="h")
})

prop.table(table(coin_toss))
grade_this()
```


2. Seven balls number 1-7 are in an urn. Two balls are drawn from the urn without replacement and the sum of $X$ of the numbers is computed. Estimate via simulation the pmf of $X$.

```{r yti2bi, exercise = TRUE}
```

*DR D*
```{r yti2bi-check}
urn <- 1:7
x <- replicate(10000, {
  balls <- sample(urn,2,replace=FALSE) 
  total_2balls <- sum(balls)
})
prop.table(table(x))
grade_this()
```


```{r yti2bi}
question("What are the least likely outcomes of $X$?",
  answer("NO1"),
  answer("NO2"),
  answer("NO3", correct = TRUE),
  answer("NO4"),
  answer("NO5"),
  allow_retry = TRUE
)
```

### Challenge Example

Suppose you have a bag full of marbles; 50 are red and 50 are blue. You are standing on a number line, and you draw a marble out of the bag. If you get red, you go left one unit. If you get blue, you go right one unit. This is called a *random walk*. You draw marbles up to 100 times, each time moving left or right one unit. Let $X$ be the number of marbles drawn from the bag until you return to 0 for the first time. The rv $X$ is called the *first return time* since it is the number of steps it takes to return to your starting position.


Estimate the pmf of $X$.

```{r challenge, exercise = TRUE}

```

*Dr. D check*

```{r challenge-check}
bag <- c(rep(-1, 50), rep(1, 50))
replicate(1000,{
steps <- sample(bag)
steps[1:10]
walk <- cumsum(steps)
walk[1:10]
where.zero <- which(walk==0)
ans<-min(where.zero)
answer.space
})
grade_this({
  if (.result > min(answer.space) & 
      .result < max(answer.space)) {
    pass("Great work!")
  }
  
  if (.result > max(answer.space)) {
    fail("Your answer is a bit high")
  }
  if (.result < min(answer.space)) {
    fail("Your answer is a bit low")
  }
  fail()
})
```


### Example: Blood types

In the United states, human blood comes in four types: O,A,B,AB. Take a sample of thirty blood types with the following probabilities: $P(O) = 0.45, P(A) = 0.4, P(B) = 0.11, P(AB) = 0.04$ 

```{r ex5a, exercise = TRUE}
bloodtypes <- c(_____,_____,_____,_____)
prob_bloodtypes <- c(_____,_____,_____,_____)
sample_blood <- sample(x =________________, size =_____, prob=________, replace=_____)
sample_blood[1:10] #quick peek to confirm
```

```{r ex5a-solution}
bloodtypes <- c(O,A,B,AB)
prob_bloodtypes <- c(0.45,0.4,0.11,0.04)
sample_blood <- sample(x =bloodtypes_, size = 30, prob=prob_bloodtypes, replace=TRUE)
sample_blood[1:10]
```

```{r ex5a-check}
grade_code()
```

The estimated pmf is then: 

```{r ex5b, exercise = TRUE}
proportions(table(sample_blood)) 
```

```{r ex5b-solution}
proportions(table(sample_blood)) 
```

```{r ex5b-check}

```


### You try it

Suppose the proportion of M&Ms by color is: 14% yellow, 13% Red, 20% Orange, 12% Brown, 20% Green, and 21% Blue. Answer the following questions using simulation. 

a. What is the probability that a randomly selected M&M is not green?

```{r yti3a, exercise = TRUE}

```

*DR D*
```{r yti3a-check}
yti3a.answer.space<-___
grade_this({
 
})

```


b. What is the probability that a randomly selected M&M is red, orange, or yellow? 

```{r yti3b, exercise = TRUE}

```    

```{r yti3b-check}
yti3b.answer.space<-___
grade_this({
 
})

```

## Submit your work

Part of your grade in this class comes from submitting tutorial work like this. 

Click the <kbd>Generate</kbd> button below to generate a hash for you assignment. This long string of numbers is a unique code for you, and this assignment. 

Paste this code into the Google Form below to submit your hash to the instructional team for scoring. You also have the opportunity to provide feedback and thoughts about the specific lesson. This is valuable information for the instructional team to know what topics were confusing for students or where extra help is needed. 

```{r context="server"}
encoder_logic()
```

```{r encode, echo=FALSE}
encoder_ui(default_ui(url = ""))
```














